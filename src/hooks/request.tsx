...
export const useRequest = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (
        !request.resident ||
        (params.residentName &&
          request.resident.residentName !== params.residentName)
      ) {
        req.resident =
          (await api.resident(params.residentName)) || req.requester;
      }

      if (
        !request.community ||
        (params.communityName &&
          request.community.communityName !== params.communityName)
      ) {
        req.community = await api.community(
          params.communityName ||
            req.resident?.communityName ||
            req.requester?.communityName
        );
      }
  ...
  return {
    ...
    isViewOnly: () => {
      ...
      return (
        !request.requester ||
        request.requester?.residentName !== request.resident?.residentName
      );
    },
    tags: () => {
    ...
    pathways: () => {
      return request.query && request.query.pathways
        ? getPathways(request.query.pathways)
        : [];
    },
  };
};
...
const api = {
  ...
  community: (communityName?: string, query?: CommunityQuery) => {
    ...
    if (!communityName) {
      ...
      return api(
        "GET",
        `/identity/v0/communities/${communityName}`,
        query
