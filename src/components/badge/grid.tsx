...
const CoordinateGrid: FunctionComponent<BadgeGridProps> = (props) => {
    ...
    const [complete, isCompleteLoading] = useBadges(props.username, {complete: true})
    const [incomplete, isIncompleteLoading] = useBadges(props.username, {incomplete: true})
    const [inactive, isInactiveLoading] = useBadges(props.username, {inactive: true})
    const badges = [...complete, ...incomplete, ...inactive]
    const isLoading = isCompleteLoading || isIncompleteLoading || isInactiveLoading
            ...
            {badges.length > 0 && (
                <div className={`grid lg:grid-cols-${columns} gap-3`}>
                        ...
                        return (
                            ...
                            >
                                <div className="h-48 w-48 m-auto overflow-hidden">
                                    <img className="h-full w-full object-contain" alt={badge.name} src={badge.imageURL}/>
                                </div>
                    ...
                    {incomplete.map((badge) => {
                        return (
                            ...
                            >
                                ...
                                <div className="h-48 w-48 m-auto overflow-hidden">
                                    <Icon className="h-full w-full stroke-gray-500 fill-gray-500" icon="badge"/>
                                </div>
                                <p className="font-semibold capitalize text-center text-gray-600 text-sm mt-3">
                                    {badge.name}
                                </p>
                    ...
                    {inactive.map((badge) => {
                        return (
                            ...
                            >
                                ...
                                <div className="h-48 w-48 m-auto overflow-hidden">
                                    <Icon className="h-full w-full stroke-gray-300 fill-gray-300" icon="badge"/>
                                </div>
                    ...
                    [...Array(gridSize - badges.length).keys()].map((k) => {
                        return (
                            <div key={"missing.badge." + k} className="bg-gray-100 px-2 py-4 rounded-md text-gray-500 h-48"/>
                        );
                    })}
                </div>
            )}
