...
export const Question = (props: QuestionProps) => {
    const [imageURL, setImageURL] = React.useState(props.answer);
    const onImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onloadend = () => {
            if (typeof reader.result === "string") {
                ...
                props.onAnswerChange && props.onAnswerChange(file)
            }
        };
 ...
 * @constructor
 */
const Response = (props: QuestionProps & {questionType: number, imageURL?: string, onImageUpload?: (e: React.ChangeEvent<HTMLInputElement>) => void}) => {
    ...
    switch (props.questionType){
        case QuestionType.FreeForm:
            return <div>
                    <textarea
                        maxLength={3000}
                        onChange={(e) => props.onAnswerChange && props.onAnswerChange(e.target.value)}
      ...
      focus:invalid:border-pink-500 focus:invalid:ring-pink-500"
        ...
        case QuestionType.MultipleChoice:
            return <fieldset className="grid grid-cols-1 gap-3">
                {
                    props.options?.map((option, i) => (
                        <div key={option} className="flex text-slate-700 text-md items-center gap-2">
                            <input
                                ...
                                onChange={() => props.onAnswerChange && props.onAnswerChange(option)}
            ...
            return <div className="grid grid-cols-1 gap-4">
                <div className="flex items-center space-x-3">
                    <div className="shrink-0">
                        <div className={`${(imageURL || props.answer) ? "rounded-full -ml-4 p-2" : "rounded-full bg-gray-100 -ml-3 p-2"}`}>
                            <img
                                className={(imageURL || props.answer) ? "h-8 w-8 object-cover rounded-full" : "h-6 w-6 object-cover"}
                                src={imageURL || props.answer || "https://cdn.localcivics.io/hub/camera.png"}
