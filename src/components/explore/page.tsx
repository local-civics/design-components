...
export const ExplorePage = () => {
    ...
    const sponsored = useEvents(req.community?.communityName, {order: "sponsored", limit: 3})
    const soonest = useEvents(req.community?.communityName, {order: "soonest", limit: 3})
    const filtered = useEvents(req.community?.communityName, eventQuery)

    const onEventClick = (eventName?: string) => req.navigate(`/communities/${req.community?.communityName}/explore/events/${eventName}`)
    ...
    return (
        <main className="h-screen max-w-full overflow-x-hidden bg-white font-proxima">
            <NavigationBar community={req.community} resident={req.resident} page="explore" />
            <ExploreComponent
