...
export const DateWidget = (props: DateWidgetProps) => {
  ...
  for (
    ...
    const base =
      "w-8 h-8 text-center cursor-pointer p-2 hover:text-white hover:font-semibold hover:bg-sky-400";
    const currentMonth =
      day && isSameDay(day, cur)
        ? "text-white font-semibold bg-sky-400"
        : isSameDay(today, cur)
        ? "font-semibold bg-slate-100 rounded-full"
      ...
      <div className="h-full">
        ...
        <div className="p-2 mt-2">
          ...
          <div className="mt-4 grid grid-cols-7 gap-1 text-xs justify-items-center">
            {weekdays.map((day, i) => {
              return (
                <span key={day + i} className="w-8 text-center text-slate-400">
                  {day}
                </span>
              );
            })}
...
const isSameDay = (compare: Date, to: Date) => {
  return (
    compare.getFullYear() === to.getFullYear() &&
