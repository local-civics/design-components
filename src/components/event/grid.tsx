...
import {EventQuery}               from "@local-civics/js-gateway";
...
const VerticalGrid: FunctionComponent<EventGridProps> = (props) => {
    const [events, isLoading] = useEvents(props.username, props.query)
    return (
        <Loader isLoading={isLoading}>
            ...
            {events.length > 0 && (
                <div className={`m-auto grid lg:grid-cols-1 gap-3 lg:max-w-[48rem]`}>
                    {events.map((event) => {
                        return (
                            <div key={event.eventId} className={`border-2 border-gray-200 rounded-md cursor-pointer transition ease-in-out text-gray-500 h-96`}>
                                <div className="w-full py-4 px-4">
                                    <Icon className="inline-block w-7 h-7 stroke-gray-700 fill-gray-700" icon={icon(props.type, event.tags, "area:")}/>
                                    <div className="align-middle ml-2 inline-block">
                                        ...
                                        <div className="text-xs">
                                            <p className="inline-block capitalize text-gray-700">
                                                {area(props.type, event.tags)}
                                            </p>
                                            {event.points && <p className="ml-1 font-semibold inline-block text-green-500">
                                                {event.points} pts
                                            </p>}
                                        </div>
                                    </div>
                                </div>
                                <img className="w-full object-cover h-64" alt={event.name} src={event.image}/>
            ...
            <div className={`grid lg:grid-cols-${columns} gap-3`}>
              {events.map((event) => {
                return (
                    <div
                        key={event.eventId}
                        className={`flex flex-col cursor-pointer transition ease-in-out ${background(area(props.type, event.tags))} p-4 rounded-md text-gray-500 h-48`}
                    >
                      <Icon
                          className="w-12 h-20 stroke-gray-700 fill-gray-700 mb-5"
                          icon={icon(props.type, event.tags, "area:")}
                      />
              ...
              [...Array(gridSize - events.length).keys()].map((k) => {
                return (
                    <div key={"missing.event." + k} className="bg-gray-100 px-2 py-4 rounded-md text-gray-500 h-48"/>
                );
              })}
            </div>
        )}
