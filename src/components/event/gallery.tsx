...
export const EventGallery = (props: EventGalleryProps) => {
  ...
  return (
    <article
      className={["grid grid-cols-1 md:flex gap-2", props.className || ""].join(
        " "
      )}
    >
          ...
          >
            ...
            <div className="absolute bottom-0 w-full">
              <div className="p-5 bg-gray-800/75 w-full">
                <div className="text-sm font-bold text-white w-full">
                  {event.title}
                </div>
                <div className="mt-2 flex items-center gap-x-2 w-full">
                  <Icon
                    className="flex-shrink-0 w-6 h-6 stroke-white fill-white"
                    icon={event.pathway || "globe"}
                  />
                  <span className="grow">
                    <div className="text-xs font-light capitalize text-white w-full">
                      {event.pathway}
                    </div>
                  </span>
