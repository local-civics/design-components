    ...
    const onUploadCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Papa.parse(file, {header: true, skipEmptyLines: true, complete: (results) => {
                ...
                if(results.errors.length > 0 || results.data.length === 0){
                    setError(<span className="text-red-500">Bad CSV, please try again or contact an admin.</span>)
                    return
                }
                ...
                results.data.forEach((row: any, i) => {
                    if(!row.email || !row.givenName || !row.familyName){
                        if(!error){
                            setError(<span className="text-red-500">Bad CSV row on line {i+1}</span>)
                        }
                        return
                    }
                })
...
const AddManually = (props: AddStudentsProps) => {
    ...
    return <form className="bg-white p-5 grid grid-cols-1 gap-y-4 rounded-md m-auto text-gray-600 min-w-[30rem]" onSubmit={onSubmit}>
