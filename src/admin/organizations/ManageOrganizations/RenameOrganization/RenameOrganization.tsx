 ...
 */
export type RenameOrganizationProps = {
    displayName?: string
...
export const RenameOrganization = (props: RenameOrganizationProps) => {
    ...
    return <Overlay onClick={props.onCancel}>
        { success && <FormDialog
            ...
            description="We've received your request to rename the organization. Please allow up to 5 mins. for changes to complete."
        /> }
        { !success && <form onClick={stopPropagation} className="bg-white p-5 grid grid-cols-1 gap-y-4 rounded-md m-auto text-zinc-600 min-w-[30rem]" onSubmit={onSubmit}>
            <h1 className="text-xl my-auto grow font-bold">Rename organization?</h1>
            <FormInput
                required
                displayName="Name"
