...
import { Topic, TopicProps } from "../../components/LearningForm/Topic/Topic";
    ...
    <div>
        ...
        <div className="flex md:w-max h-screen gap-x-2 justify-items-center content-center m-auto">
          <div
            ...
            className={`text-slate-300 hover:text-slate-500 w-8 h-8 md:w-14 md:h-14 cursor-pointer my-auto ${
              stage === LearningFormStage.Start ? "invisible" : ""
            }`}
          >
            <Icon name="leftArrow" />
          </div>
      ...
      {exit && (
        <div className="fixed top-0 left-0 px-4 md:px-2 w-screen h-screen py-5 transition ease-in-out duration-400 bg-gray-200/75 z-50">
          <div className="flex md:w-max h-screen gap-x-2 justify-items-center content-center m-auto">
            <ConfirmExit onYes={props.onClose} onNo={() => setExit(false)} onClose={() => setExit(false)} />
          </div>
        </div>
      )}
    </div>
...
const Stage = (props: LearningFormWorkflowProps & { stage: number; question: number }) => {
  ...
  switch (stage) {
    ...
    case LearningFormStage.Topic:
      return <Topic {...props.topic} isLoading={props.isLoading} onClose={props.onClose} />;
    case LearningFormStage.Question:
      if (!props.questions || props.questions.length === 0) {
        return null;
      }
    ...
    case LearningFormStage.Submit:
      return <Submit isLoading={props.isLoading} onClose={props.onClose} onSubmit={props.onSubmit} />;
  }
