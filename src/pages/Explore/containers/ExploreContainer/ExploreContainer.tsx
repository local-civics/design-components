      ...
      <Gallery
        ...
        primary={
          primary && (
            <ExperienceComponent
              {...primary}
              onClick={() => navigate(`${location.pathname}/${primary.id}`)}
            />
          )
        }
        top={
...
const useExperiences = (pathways?: string[]) => {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      setExperiences(await api.curriculum.viewMarketplaceActivities(primaryOrganization.nickname, {
        name: search || name || "",
        tags,
        skills,
        pathways,
    ...
    return () => { setExperiences(null); setResolving(false) }
