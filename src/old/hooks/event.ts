 ...
 */
export const useEvents = (courseName?: string | null, query?: EventQuery | null) => {
  const { api } = useApi();
  const [events, setEvents] = useState(null as Event[] | null);
  useEffect(() => {
    (async () => {
      setEvents(null);
      if (courseName && query !== null) {
 ...
 */
export const useEventQuery: (from: object | null) => [EventQuery | null, (key: string, value: any) => void] = (
  from: object | null
) => {
  ...
  return [
    ...
    (key: string, value: any) => {
      ...
      const isNullQuery = !(
        (newQuery.tags && newQuery.tags?.length > 0) ||
        (newQuery.pathways && newQuery.pathways?.length > 0) ||
        newQuery.title
      );
      if (isNullQuery) {
        setQuery(null);
 ...
 */
export const useEvent = (courseName?: string, eventName?: string, query?: EventQuery | null) => {
  const { api } = useApi();
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!courseName || !eventName) {
        ...
        setEvent(await api("GET", `/curriculum/v0/courses/${courseName}/events/${eventName}`, query || undefined));
      }
    })();
