 ...
 * @constructor
 */
export const BadgeComponent: FunctionComponent<BadgeComponentProps> = (props) => {
        ...
        <div className="w-full">
          <Loader isLoading={isLoading}>
            <div className="w-full pl-5 pr-10 pb-5 border-b border-gray-200">
              <Icon className="inline-block w-7 h-7 stroke-slate-600 fill-slate-600" icon="shield" />
              <div className="align-middle ml-2 inline-block">
                <p className="font-semibold capitalize text-slate-600 text-lg">{badge.title}</p>
                <div className="text-xs">
                  <p className="inline-block text-slate-600">{badge.summary}</p>
                  {badge.proficiency && (
                    <p className="ml-1 font-semibold inline-block text-blue-500">{badge.proficiency} pts</p>
                  )}
                </div>
              </div>
            </div>
            ...
            {badge.criteria && badge.criteria.length > 0 && (
              <div className="w-full mt-5 px-5">
                <p className="text-sm mt-5 font-bold text-gray-600">Objectives</p>
                <div className="mt-5 grid gap-10 w-full">
                  {badge.criteria.map((criterion, i) => {
                    ...
                    const border = criterion.completedAt
                      ...
                      : "border rounded border-gray-200";
                    const opacity = badge.status === "unqualified" || criterion.completedAt ? "opacity-50" : "";
                    const icon: IconName = criterion.completedAt ? "accept" : "circle";
                    const cursor =
                    ...
                    return (
                      <div
                        ...
                        onClick={() => props.onObjectiveClick(criterion.actionURL)}
                        className={`flex px-3 py-2 gap-5 items-center ${color} ${cursor} ${border} ${opacity}`}
                      >
                        <Icon className={`h-3 w-3 stroke-inherit fill-inherit`} icon={icon} />
                        <div className="grow">
                          <div className="align-middle ml-2 inline-block">
                            <p className="font-semibold text-gray-600 text-md">{criterion.title}</p>
                          </div>
                        </div>
                      </div>
                    );
