...
/**
 * Pure presentational event overview
 * @constructor
 */
export const EventOverview: FunctionComponent<EventOverviewProps> = (props) => {
  ...
  const cta = (() => {
    switch (event.status) {
      ...
      case "contributed": // update status
        return (
          <button className="transition-colors shadow-md font-semibold rounded-lg max-h-14 py-3 px-12 bg-slate-700 text-white mx-2 lg:mt-2">
            Revisit
          </button>
      ...
      <div className="pb-5 h-full w-full">
        ...
        <div className="w-full h-full min-h-60 justify-items-center content-center">
          <Loader isLoading={isLoading}>
            <img className="w-full h-60 object-cover" alt={event.title} src={event.imageURL} />
            <div className="w-full flex p-5 border-b border-gray-200">
              <div className="flex items-start grow">
                ...
                <div className="grow align-top ml-2 inline-block leading-none">
                  <p className="font-semibold capitalize text-slate-600 text-lg -mt-1.5">{event.title}</p>
                  <div>
                    <p className="text-xs inline-block capitalize text-slate-600">{event.pathway}</p>
                    {event.proficiency && (
                      <p className="ml-1 font-semibold inline-block text-xs text-green-500">{event.proficiency} pts</p>
                    )}
                  </div>
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-1 gap-8">
                      {event.location && (
                        <div className="flex gap-1 items-center">
                          <Icon className="grow-0 w-6 h-6 min-w-6 stroke-slate-600 fill-slate-600" icon="pin" />
                      ...
                      {event.notBefore && (
                        <div className="flex gap-1 items-center">
                          <Icon className="grow-0 w-6 h-6 min-w-6 stroke-slate-600 fill-slate-600" icon="clock" />
                      ...
                      {event.url && (
                        <div className="flex gap-1 items-center">
                          <Icon className="grow-0 w-6 h-6 min-w-6 stroke-slate-600 fill-slate-600" icon="globe" />
...
const getSkills = (event: Event) => {
  return event.tags?.map((tag) => (tag.startsWith("skill:") ? tag.replace("skill:", "") : "")).filter((tag) => tag);
};
