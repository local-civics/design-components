...
export const PathwayWidget: FunctionComponent<PathwayWidgetProps> = (props) => {
  ...
  const pathways: { name: Pathway; readiness: Report | null }[] = [
    {
    ...
    >
      ...
      <div className="p-2">
        <div className="flex items-center">
          <div className="grow">
            <Icon className="w-5 h-5 stroke-slate-600 fill-slate-600 inline-block" icon="pathway" />
            <h4 className="ml-2 capitalize align-middle font-semibold text-slate-600 inline-block">{props.title}</h4>
          </div>
          <Icon
              ...
              return (
                <div key={pathway.name} className="mt-5 flex items-center">
                  ...
                  <div className="grow ml-2">
                    <p className="capitalize text-xs text-slate-400">{pathway.name}</p>
                    <ProgressBar
                      className="h-3"
