 ...
 * @constructor
 */
export const PathwayHelpComponent: FunctionComponent<PathwayHelpComponentProps> = (props) => {
    ...
    <div className="grid grid-cols-1 justify-items-center content-center transition ease-in-out duration-300 fixed top-0 w-screen h-screen p-5 bg-gray-500/80 z-50">
      <div className="shadow-md p-5 w-80 lg:w-[28rem] h-64 lg:h-80 bg-white rounded-md grid grid-cols-1 justify-items-center">
        ...
        <div className="grid grid-cols-1 m-auto">
          <Icon className="m-auto stroke-slate-600 fill-slate-600 w-12 lg:w-16" icon="pathway" />
          <div className="h-28">
            <p className="w-max m-auto text-slate-600 font-bold text-md lg:text-lg mt-5">{title}</p>
            <p className="max-w-72 h-16 m-auto text-center text-gray-500 text-xs lg:text-sm mt">{description}</p>
          </div>
