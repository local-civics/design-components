...
export const EngagementWidget: FunctionComponent<EngagementWidgetProps> = (props) => {
  ...
  const onBadgeClick = (badgeName: string) => navigate(`${location.pathname}/badges/${badgeName}`);

  let tab = null;
  switch (props.active) {
    case "milestones":
      tab = <MilestoneTab community={props.community} events={props.milestones} onClick={props.onEventClick} />;
      break;
    case "activity":
      tab = <ActivityTab community={props.community} events={props.activity} onClick={props.onEventClick} />;
      break;
    case "badges":
      tab = (
        <BadgeTab
        ...
        <div className="grid gap-2 grid-cols-1 lg:grid-cols-3 justify-items-center">
          <div
            ...
            className={`transition ease-in-out w-full rounded-md py-3 px-5 ${
              props.active === "badges" ? "bg-white" : "cursor-pointer hover:bg-white"
            }`}
          >
            <div className="lg:m-auto w-max">
              <Icon className="w-5 h-5 stroke-slate-600 fill-slate-600 inline-block" icon="badges" />
              <h4 className="ml-1 capitalize text-sm font-semibold align-middle text-slate-600 inline-block">Badges</h4>
            </div>
          </div>
          <div
            onClick={() => setActive("milestones")}
            className={`transition ease-in-out w-full rounded-md py-3 px-5 ${
              props.active === "milestones" ? "bg-white" : "cursor-pointer hover:bg-white"
            }`}
          >
            <div className="lg:m-auto w-max">
              <Icon className="w-5 h-5 stroke-slate-600 fill-slate-600 inline-block" icon="milestones" />
          ...
          <div
            ...
            className={`transition ease-in-out w-full rounded-md py-3 px-5 ${
              props.active === "activity" ? "bg-white" : "cursor-pointer hover:bg-white"
            }`}
          >
            <div className="lg:m-auto w-max">
              <Icon className="w-5 h-5 stroke-slate-600 fill-slate-600 inline-block" icon="activity" />
