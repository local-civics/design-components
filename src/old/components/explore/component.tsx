...
export const ExploreComponent = (props: ExploreComponentProps) => {
  ...
  const hasFiltered = props.hasFilter && filtered && filtered?.length > 0;

  const mostRecommended = props.top?.slice(0, 1)[0];
  const hasMostRecommended = !props.hasFilter && mostRecommended;

  const recommended = props.top?.slice(1);
  const hasRecommended = !props.hasFilter && recommended && recommended.length > 0;

  const sponsored = props.sponsored;
  const hasSponsored = !props.hasFilter && sponsored && sponsored.length > 0;

  const soonest = props.soonest;
  const hasSoonest = !props.hasFilter && soonest && soonest.length > 0;
    ...
    <Loader isLoading={props.resident === null}>
      <article className="px-4 lg:px-24 py-5">
        ...
        <div className="lg:flex w-full mt-5">
          {/* Left Panel */}
          <div className="lg:flex lg:flex-col h-full w-full lg:w-60">
          ...
          <div className="lg:flex lg:flex-col lg:ml-9 w-full max-w-full lg:px-2 overflow-x-hidden">
            ...
            <label className="relative block">
              <p className="mb-3 text-slate-600 font-semibold text-md">Find Events</p>
              ...
              <input
                onChange={(e) => props.onEventSearch(e.target.value)}
                className="placeholder:italic placeholder:text-slate-400 text-slate-500 block bg-white w-full border border-slate-300 rounded-md py-2 pl-8 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm"
                type="text"
                name="search"
                placeholder="Search for community events..."
              />
            </label>
            ...
            <Loader
              isLoading={props.top === null || props.soonest === null || (props.hasFilter && props.filtered === null)}
            >
              {body}
            </Loader>
          </div>
