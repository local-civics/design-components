...
import { FileLockerProps } from "./FileLocker";
...
export type FileLockerSearchModalProps = {
    opened: boolean
    onClose: () => void
    pathways: NonNullable<FileLockerProps["pathways"]>
    badges: NonNullable<FileLockerProps["badges"]>
    lessons: FileLockerProps["lessons"]
 ...
 * File Locker's own grouping tabs first. Reuses the exact pathways/badges/lessons arrays
 * FileLocker already has and the same onPathwayClick/onBadgeClick/onLessonClick callbacks its own
 * group-card titles use - zero new data fetching. Filters all 3 lists client-side by a single
 * search term; picking a result both fires the matching callback and closes the modal, matching
 * CategoriesModal's own onCategoryClick convention.
 * @param props
 * @constructor
 */
export function FileLockerSearchModal(props: FileLockerSearchModalProps) {
    ...
    return (
        <Modal
            ...
            title={<span className="text-base font-extrabold text-dark-blue-400">Find a Pathway, Badge, or Lesson</span>}
            size="lg"
            centered
        >
