...
export type StackData = {
    items: Item[]
    // Carried straight through to every row's <Link state={...}> - see Pathway/BadgeStack.tsx's
        ...
        <div className="flex flex-col gap-2">
            {props.items.map((row) => (
                <div key={row.lessonName} className="flex items-center justify-between gap-4 py-1.5">
                    <Link to={row.href} state={props.state} className="text-sm font-bold text-dark-blue-400 no-underline hover:underline">{row.lessonName}</Link>
